 ...
 * @Last Modified time: 2018-09-25 18:00:45
 */
/**
|--------------------------------------------------
| 描述: 通用表头组件
...
*/
<template>
    <div class="bim-header-info">
        <van-row>
            <van-col span="12">
                <slot name="left"></slot>
                <div v-if="!$slots.left" class="bim-header-info__name">{{ bimHeaderMyInfoObjName }}</div>
            </van-col>
            <van-col span="12">
                <slot name="right"></slot>
                <div v-if="!$slots.right" class="bim-header-info__date">{{ bimHeaderMyInfoObjDate }}</div>
            </van-col>
        </van-row>
    </div>
</template>
