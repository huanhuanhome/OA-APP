 ...
 * @Last Modified time: 2018-09-25 10:42:24
 */
    ...
    <div class="bimcalendar">
        ...
        <div class="bimcalendar-month">
            <ul class="bimcalendar-month-ul">
                ...
                <li class="arrow-right" v-if="buttonDisable" @click="pickNext(currentYear, currentMonth)"><i class="iconfont icon-right" style="color:#fff;"/></li>
                <li class="arrow-right" v-else><i class="iconfont icon-right" style="color:transparent;"/></li>
            </ul>
        </div>
        <!-- 星期 -->
        <ul class="weekdays">
...
export default {
    ...
    computed: {
        balance() {
            return getMappingCfg()['SingConfiguration.BalanceDay'].date.value
        },
        initMonth() {
            const day = new Date().getDate()
            if (Number(day) > Number(this.balance)) {
                return `${new Date().getFullYear()}/${new Date().getMonth()}/1`
            } else {
                return `${new Date().getFullYear()}/${+new Date().getMonth() - 1}/1`
            }
        },
        buttonDisable() {
            // 不初始化月份
            if (!this.isInitMonth) return true
            if (this.currentYear === new Date().getFullYear()) {
                return this.currentMonth < (new Date(this.initMonth).getMonth() + 1)
            } else {
                return true
            }
        }
    ...
    methods: {
        ...
        initData(cur) {
            ...
            if (cur) {
                ...
                // 判断初始化月份
                const now = this.buttonDisable ? new Date(this.initMonth) : new Date()
            ...
            this.$emit('month-click', this.currentYear, this.currentMonth)
            this.mergeStatus()
        },
        async mergeStatus() {
            try {
